...
class _AddVehicleState extends State<AddVehicle> {
  ...
  final TextEditingController insuranceValid = TextEditingController();
  final TextEditingController registerationDateController = TextEditingController();
  final TextEditingController vehicleCategory = TextEditingController();
  final TextEditingController brand = TextEditingController();
  final TextEditingController modelType = TextEditingController();
  final TextEditingController fuelType = TextEditingController();
  final TextEditingController vehicleOwnerShip = TextEditingController();
  ...
  final GlobalKey<FormFieldState<File?>> carNoKey = GlobalKey<FormFieldState<File?>>();
  final GlobalKey<FormFieldState<File?>> insuranceCopyKey = GlobalKey<FormFieldState<File?>>();
  final GlobalKey<FormFieldState<File?>> registerCertificateFront = GlobalKey<FormFieldState<File?>>();
  final GlobalKey<FormFieldState<File?>> registerCertificateBack = GlobalKey<FormFieldState<File?>>();
  ...
  final Map<String, List<String>> brandModelMap = {
    'Maruti': ['Swift', 'Dzire', 'Ertiga', 'Maruti Suzuki Ciaz'],
    'Hyundai': ['Xcent', 'Aura', 'i10', 'i20'],
    'Honda': ['Amaze', 'Honda City'],
    'Toyota': ['Etios Liva', 'Etios-GD', 'Innova', 'Innova crysta', 'Innova Hycross'],
    'KIA': ['Kia Seltos', 'Kia Sonet', 'Kia Carnes', 'Kia Carnival'],
    'Tata': ['Tata Zest'],
    'BYD': ['BYD E6'],
    'MG': ['MG ZS', 'MG ZS EV', 'MG HECTOR']
  };
  ...
  void initState() {
    super.initState();
    // Initialize _modelOptions with the models of the default selected brand
    _modelOptions = brandModelMap[selectedbrand] ?? [];
    selectedModel = _modelOptions.isNotEmpty ? _modelOptions[0] : '';
  }
                  ...
                  child: Column(
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          Text('Select Id Proof', style: TextStyle(fontWeight: FontWeight.bold)),
                      ...
                      DropdownInputField(
                        ...
                        onChanged: (String? newValue) {
                          setState(() {
                            ...
                            _modelOptions = brandModelMap[selectedbrand] ?? [];
                            // Set selectedModel to the first item if available, or empty
                            selectedModel = _modelOptions.isNotEmpty ? _modelOptions[0] : '';
                          });
                          print('Selected: $newValue');
                        },
                        ...
                        child: PrimaryButton(
                          ...
                          onPressed: () async {
                            if (formKey.currentState!.validate()) {
                              ...
                              final String insurancePicPath = insurenceCopy?.path ?? '';
                              final String registerCertificatFPath = registerCertificateF?.path ?? '';
                              final String registerCertificateBPath = registerCertificateB?.path ?? '';
                              final String carNoPicPath = carNoPhoto?.path ?? '';

                              addVehicleController.addVehicleDetails(
                                  vehicleNo.text.trim(),
                                  selectedbrand,
                                  selectedFuelValue,
                                  insuranceValid.text.trim(),
                                  registerationDateController.text.trim(),
                                  carNoPicPath,
                                  selectedVehicleCategory,
                                  selectedModel,
                                  selectedVehicleOwner,
                                  insurancePicPath,
                                  registerCertificatFPath,
                                  registerCertificateBPath,
                                  context);
                            }
                          },
                        ),
